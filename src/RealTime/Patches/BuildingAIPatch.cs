...
namespace RealTime.Patches
{
    ...
    internal static class BuildingAIPatch
    {
        ...
        [HarmonyPatch]
        private sealed class CampusBuildingAI_HandleCrime
        {
            ...
            private static readonly CommonBuildingAIHandleCrimeDelegate BaseHandleCrime = AccessTools.MethodDelegate<CommonBuildingAIHandleCrimeDelegate>(typeof(CommonBuildingAI).GetMethod("HandleCrime", BindingFlags.Instance | BindingFlags.NonPublic), null, true);

            [HarmonyPatch(typeof(CampusBuildingAI), "HandleCrime")]
            [HarmonyPrefix]
            public static bool HandleCrime(CampusBuildingAI __instance, ushort buildingID, ref Building data, int crimeAccumulation, int citizenCount)
            {
                var instance = Singleton<DistrictManager>.instance;
                ...
                if (b != 0)
                {
                    if (!instance.m_parks.m_buffer[b].IsCampus)
                    {
                        b = 0;
                    }
                    else if (__instance.m_campusType == DistrictPark.ParkType.GenericCampus || __instance.m_campusType != instance.m_parks.m_buffer[b].m_parkType)
                    {
                        b = 0;
                    }
                }
                ...
                if (crimeAccumulation != 0)
                {
                    ...
                    {
                        var instance3 = Singleton<EventManager>.instance;
                        var info = instance3.m_events.m_buffer[data.m_eventIndex].Info;
                ...
                var problemStruct = Notification.RemoveProblems(data.m_problems, Notification.Problem1.Crime);
