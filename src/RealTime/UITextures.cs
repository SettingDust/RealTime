namespace RealTime
{
    ...
    public static class UITextures
    {
        ...
        private static readonly Dictionary<string, UITextureAtlas> TextureCache = [];

        // Dictionary to cache texture file lookups.
        private static readonly Dictionary<string, UITextureAtlas> FileCache = [];
        ...
        {
            var cursor = ScriptableObject.CreateInstance<CursorInfo>();
            ...
            get
            {
                ...
                foreach (var item in plugins)
                {
                    try
                    {
                        ...
                        if (instances.FirstOrDefault() is not RealTimeMod)
                        {
                            continue;
                        }
                        return item;
        ...
        public static UITextureAtlas LoadSprite(string fileName)
        {
            ...
            var newTexture = LoadTexture(fileName + ".png");
        ...
        /// <param name="fileName">Atlas file name (".png" will be appended to make the filename).</param>
        /// <returns>New texture atlas.</returns>
        public static UITextureAtlas LoadSingleSpriteAtlas(string fileName) => LoadSpriteAtlas(fileName, ["normal"]);
        ...
        private static Texture2D LoadTexture(string fileName)
        {
            try
            {
                using var stream = OpenResourceFile(fileName);
                // New texture.
            ...
            {
                Log.Error("exception reading texture file " + fileName);
        ...
        {
            string path = Path.Combine(PluginInfo.modPath, "Resources");
